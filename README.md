...
**Q：安装包这么小，AI 能力从哪来？**
A：AI 走云端 API（用户自备 Key），工具箱不捆绑模型。本地离线 AI 受设备条件（GPU/显存/存储）限制暂缓，条件成熟后再评估。
...
| 版本     | 定位                                                  | 状态        |
| -------- | ----------------------------------------------------- | ----------- |
| **v0.1** | 预览版：骨架 + 四大模块 + 云端 AI + Skills + 文件转换 | ✅ 已发布（v0.1.2） |
| v0.2.0   | 全局日志基础版 + HDR 截图集成 + 双轨策略验证          | 📋 下一版目标 |
| v0.3.0   | 扫雷 + 贪吃蛇 + 游戏插件模板                          | 📋 确定开发 |
| v1.0     | 正式版：架构稳定 + 主要功能就绪 + 用户验证通过        | ⬜ 不预设时间 |
...
工具箱采用**双轨并行**发布策略：值得独立存在的功能会同时以两个形态发布——**独立项目**（拉新，让不认识工具箱的人先发现它）和**工具箱集成**（留存，让已经在这里的人持续使用）。

- 🌳 **内核 + 外壳**：核心功能只写一次，放在共享库中；独立项目和工具箱各自引用、各自配置外壳，体验一致又不互相绑架
- 🆔 **版本号各自独立**：工具箱版本只标记工具箱本身的变化，与独立项目互不绑定
- 🤝 **双向互推**：独立项目 README 会提及工具箱，工具箱 Release Notes 也会提及独立项目

当前生态成员：

| 项目 | 定位 | 状态 |
| ---- | ---- | ---- |
| 🌿 Nahida-nana 工具箱 | 主产物，全功能集成 | ✅ 已发布 |
| 📷 HDR 截图（独立项目） | 双轨策略首个验证，核心库将集成进工具箱 v0.2.0 | 🌱 开发中 |

更多独立项目在生长中——判断标准只有一个：想不想让这个功能有独立的存在感。
